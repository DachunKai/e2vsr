...
def prepare_h5_keys_ced_hr(folder_path):
    ...
    for root, subdir, _ in os.walk(folder_path):
        if 'images' in subdir:
            ...
            out = osp.join('datasets/CED_3_h5/Voxel_3/HR', osp.basename(root) + '.h5')
...
def prepare_h5_keys_ced_lr(folder_path):
    ...
    for root, subdir, _ in os.walk(folder_path):
        if 'images' in subdir:
            root_path_list.append(root)
            h5_hr_path_list.append(osp.join('datasets/CED_3_h5/Voxel_3/HR', osp.basename(root) + '.h5'))
            h5_lr_path_list.append(osp.join('datasets/CED_3_h5/Voxel_3/LRx4', osp.basename(root) + '.h5'))
...
def create_hdf5_for_ced_lr():
    ...
    """
    # HR
    folder_path = 'datasets/CED_3/LRx4'
